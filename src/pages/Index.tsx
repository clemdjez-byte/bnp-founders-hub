...
const Index = () => {
          ...
          for (let j = i + 1; j < updatedBubbles.length; j++) {
            ...
            if (checkCollision(updatedBubbles[i], updatedBubbles[j])) {
              const {
                b1,
                b2
              } = resolveCollision(updatedBubbles[i], updatedBubbles[j]);
              updatedBubbles[i] = b1;
              updatedBubbles[j] = b2;
            }
          }
  ...
  const handleMouseUp = () => {
    if (draggedBubble) {
      setBubbles(prev => prev.map(b => b.id === draggedBubble ? {
        ...b,
        isDragging: false,
  ...
  return <div className="relative min-h-screen w-full overflow-hidden bg-gradient-to-br from-background via-background to-muted cursor-grab active:cursor-grabbing" onMouseMove={handleMouseMove} onMouseUp={handleMouseUp} onMouseLeave={handleMouseUp}>
      {bubbles.map(bubble => {
      const logoSrc = logoMap[bubble.logo];
      return <div key={bubble.id} className="absolute rounded-full flex flex-col items-center justify-center transition-all duration-100 cursor-move select-none hover:scale-105" style={{
        left: `${bubble.x}px`,
        top: `${bubble.y}px`,
        ...
        transform: bubble.isDragging ? 'scale(1.15)' : 'scale(1)'
            ...
            maxHeight: `${bubble.size * 0.35}px`
          }} draggable="false" className="object-fill" />
              ...
              fontSize: `${Math.max(12, bubble.size * 0.06)}px`
                ...
                <div className="text-muted-foreground font-medium drop-shadow-lg mb-1" style={{
              fontSize: `${Math.max(10, bubble.size * 0.04)}px`
            }}>
                  {bubble.type}
                </div>
                <div className="text-foreground font-bold drop-shadow-lg" style={{
